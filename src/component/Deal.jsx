...
const Deal = () => {
  ...
  const getProductsToShow = () => {
    // Show all products from start to currentIndex + productsPerPage
    return products.slice(0, currentIndex + productsPerPage);
  };
  ...
  const handleViewMore = () => {
    ...
    // Scroll to the first product of the newly added set
    const firstNewProductIndex = currentIndex;
    if (firstNewProductIndex < products.length) {
      const firstNewProductElement = document.getElementById(
        `product-${products[firstNewProductIndex].id}`
      );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div className="text-right text-sm">
          <div className="text-gray-500 mb-1">
            {Math.min(currentIndex + productsPerPage, products.length)} of{" "}
      ...
      <div className="flex justify-center gap-4">
        {currentIndex + productsPerPage < products.length && (
          <button
            ...
            className="flex items-center gap-2 bg-white hover:bg-blue-50 text-blue-600 font-semibold py-2.5 px-3 text-[11px] md:px-6 md:text-sm rounded-full border-2 border-blue-200 hover:border-blue-300 transition-all duration-300 shadow-md hover:shadow-lg "
          >
            <span>
              View More ({products.length - (currentIndex + productsPerPage)})
            </span>
